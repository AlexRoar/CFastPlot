        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                case SDL_MOUSEWHEEL:{
                    ...
                    if (graph->yRanges.positive <= 1)
                        graph->yRanges.positive = 1;
                    if (graph->yRanges.negative <= 1)
                        graph->yRanges.negative = 1;
                    graph->yRng = graph->yRanges.negative + graph->yRanges.positive;
                    graph->matchXRange();
                    graph->content.clear();
                    graph->plot(func, 10, 2);
                    graph->render();
